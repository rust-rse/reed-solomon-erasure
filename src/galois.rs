    ...
    unsafe {
        if len > 8 {
            ...
            while n < len_minus_8 {
                ...
                *out_ptr.offset(4) ^= *input_ptr.offset(4);
                *out_ptr.offset(5) ^= *input_ptr.offset(5);
                *out_ptr.offset(6) ^= *input_ptr.offset(6);
                *out_ptr.offset(7) ^= *input_ptr.offset(7);
...
mod tests {
    ...
    fn test_galois() {
        ...
        mul_slice_xor(52, &input, &mut output);
        for i in 0..input.len() {
            assert_eq!(expect_xor[i], output[i]);
        }

        mul_slice(177, &input, &mut output);
        ...
        mul_slice_xor(117, &input, &mut output);
        for i in 0..input.len() {
            assert_eq!(expect_xor[i], output[i]);
        }
